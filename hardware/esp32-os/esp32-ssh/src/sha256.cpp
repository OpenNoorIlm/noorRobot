...
void SHA256::update(const uint8_t* data, size_t len) {
  ...
  while (len > 0) {
    size_t take = std::min(len, (size_t)64 - bufferLen_);
...
std::vector<uint8_t> SHA256::digest() {
  ...
  uint8_t lenBytes[8];
  for (int i = 0; i < 8; i++) lenBytes[7 - i] = (uint8_t)(savedBitlen >> (8 * i));
  memcpy(buffer_ + bufferLen_, lenBytes, 8);
  bufferLen_ += 8;
  transform(buffer_);
  bufferLen_ = 0;
...
std::string SHA256::hex(const std::vector<uint8_t>& bytes) {
...
// HMAC-SHA256 per RFC 2104, using the SHA256 primitive above.
std::vector<uint8_t> hmacSha256(const std::vector<uint8_t>& key, const std::string& msg) {
  ...
  std::vector<uint8_t> innerDigest = inner.digest();
